            ...
            if li==():
                ...
                if npwd==self.pwd:
                    if self.user in root_user_list:
                        self.adm_win=adm()
                        self.hide()
                        self.adm_win.show()#跳转管理员界面
                    else:
                        self.cus_win=cus()
                        self.hide()
                        self.cus_win.show()#跳转客户界面
                else:
...
#添加商品----------------------------------
    ...
    def insert_pro(self):
        ...
        panp=self.select_shop(pid,shopnum)
        db=pymysql.connect(host='106.52.121.163',user='root',password='@plmnbvcxzaq0921',db='competition')
        cursor=db.cursor()
        ...
        if pan==1:            
            ...
            if panp==1:
                self.insert(sql1)
                QMessageBox.information(self,'提示','添加成功(以编号对应产品添加)')
            else:
                QMessageBox.warning(self,'错误','该产品已存在')

    def select_shop(self,pro,shopnum):
        db=pymysql.connect(host='106.52.121.163',user='root',password='@plmnbvcxzaq0921',db='competition')
        cursor=db.cursor()
        sql='''select * from 商店销售 where 产品UPC代码="%s" and 商店认证码="%s"'''%(pro,shopnum)
        cursor.execute(sql)
        li=cursor.fetchall()
        db.close()
        if li==():
